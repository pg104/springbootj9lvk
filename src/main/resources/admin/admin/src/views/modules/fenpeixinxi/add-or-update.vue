<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="专家账号" prop="zhuanjiazhanghao">
          <el-select :disabled="ro.zhuanjiazhanghao" @change="zhuanjiazhanghaoChange" v-model="ruleForm.zhuanjiazhanghao" placeholder="请选择专家账号">
            <el-option
                v-for="(item,index) in zhuanjiazhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.zhuanjiazhanghao" label="专家账号" prop="zhuanjiazhanghao">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="分配备注" prop="fenpeibeizhu">
          <el-input v-model="ruleForm.fenpeibeizhu" 
              placeholder="分配备注" clearable  :readonly="ro.fenpeibeizhu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="分配备注" prop="fenpeibeizhu">
              <el-input v-model="ruleForm.fenpeibeizhu" 
                placeholder="分配备注" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zhuanjia/zhuanjiazhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zhuanjiazhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    zhuanjiazhanghaoChange () {
      this.$http({
        url: `follow/zhuanjia/zhuanjiazhanghao?columnValue=`+ this.ruleForm.zhuanjiazhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.zhuanjiaxingming){
                this.ruleForm.zhuanjiaxingming = data.data.zhuanjiaxingming
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springbootj9lvk/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fenpeixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fenpeixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
